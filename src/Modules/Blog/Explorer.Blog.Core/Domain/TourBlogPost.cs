...
namespace Explorer.Blog.Core.Domain
{
    public class TourBlogPost
    {
            ...
            public TourBlogPost(int authorId, int tourId, string title, string description, DateTime creationDate, List<string>? imageURLs, List<BlogPostComment>? comments, BlogPostStatus status, List<BlogPostRating>? ratings)
            {
                if (authorId == 0) throw new ArgumentException("Field required");
                if (tourId == 0) throw new ArgumentException("Field required");
